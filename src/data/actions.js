...
export const ACTIONS = [
  ...
  action("pour_basin_breaker", "把盆中水泼向电闸下方", ["往电闸泼水", "把水倒在电闸旁"], c => has(c, "basin") && c.object("basin")?.state.water && isNear(c, "breaker", 150), { type: "pour", target: "breaker", flags: { breaker_floor_wet: true }, prophecy: ["P_BREAKER_DARK", "water_short"] }),
  ...
  action("camera_timer_self", "设置定时自拍记录动作连续性", ["定时自拍", "相机拍我", "记录我自己"], c => has(c, "camera"), { type: "monitor", target: "player", flags: { self_timer_chain: true }, progress: ["alone", 3] }),
  ...
  action("press_photo_back", "把拇指完整压在受潮照片背面", ["在照片背面按指印", "拇指按照片", "留下完整指纹"], c => isNear(c, "photo", 100) || has(c, "photo"), { type: "scene_interaction", target: "photo", flags: { photo_player_print: true }, prophecy: ["P_PHOTO_PRINT", "player_press"], message: "苏岚把拇指完整压在照片背面。指腹的水、显影液和压力都留在纸纤维里。" }),
  ...
  action("breaker_off", "切断总电源", ["关电闸", "切断电源", "拉闸断电"], c => isNear(c, "breaker", 105) && c.state.environment.power, { type: "power", value: false, flags: { breaker_off: true }, progress: ["breach", 1], prophecy: ["P_BREAKER_DARK", "player_cut"] }),
  ...
  action("roof_yao_turn_from_glass", "让苏遥转身避开碎玻璃但继续看镜中反射", ["让苏遥避开玻璃", "用反射看我", "苏遥转身继续看"], c => c.state.sceneId === "roof_yao" && c.nearNpcIds.includes("yao"), { type: "scene_interaction", target: "yao", flags: { yao_reflection_witness: true, yao_eyes_closed: false }, progress: ["yao", 2], trust: ["yao", 1], message: "苏遥背对碎玻璃，从相机反光板里继续确认苏岚的位置。她看见的是反射，不是本人。" }),
  action("roof_luhui_cross_gap", "与陆洄用共绳跨过屋顶断口", ["和陆洄过断口", "用绳跨屋顶", "陆洄拉我过去"], c => c.state.sceneId === "roof_luhui" && c.state.flags.rope_with_luhui, { type: "scene_interaction", target: "rope", flags: { luhui_mutual_alibi: true, roof_gap_crossed: true }, progress: ["luhui", 2], trust: ["luhui", 1], prophecy: ["P_ROPE_TAUT", "sulan_luhui"], message: "两人的重量轮流压上同一根绳；谁先松手会留在绳结和屋脊擦痕里。" }),
  action("roof_alone_order_floats", "用身体和绳结固定漂浮物顺序", ["固定漂浮物顺序", "用身体挡住漂浮物", "做漂流连续链"], c => c.state.sceneId === "roof_alone" && c.state.flags.rope_on_player, { type: "scene_interaction", target: "rope", trace: "float_order", flags: { embodied_chain: true, float_order_fixed: true }, progress: ["alone", 2], message: "漂浮物依次撞上绳和苏岚的身体，顺序成为无人见证时仍连续的物理记录。" }),
  action("darkroom_yao_split_view", "让苏遥同时对照监控画面与门缝实物", ["对照监控和实物", "让苏遥看两个画面", "检查监控延迟"], c => c.state.sceneId === "darkroom_yao" && c.nearNpcIds.includes("yao"), { type: "scene_interaction", target: "camera", flags: { yao_witness_chain: true, screen_delay_known: true }, progress: ["yao", 2], trust: ["yao", 1], message: "苏遥指出监控比门缝里的动作慢半拍。此后‘看见画面’不能再自动等于‘看见本人’。" }),
  action("darkroom_luhui_swap_controls", "与陆洄交换泵和总闸的看守位置", ["和陆洄交换位置", "我守泵他守闸", "交换泵和电闸"], c => c.state.sceneId === "darkroom_luhui" && c.nearNpcIds.includes("luhui"), { type: "scene_interaction", target: "pump", flags: { luhui_mutual_alibi: true, controls_swapped: true }, progress: ["luhui", 2], trust: ["luhui", 1], message: "两人交换控制点。交换本身占用时间，也使任何一人都无法独自声称同时守住泵和总闸。" }),
  action("darkroom_alone_insulate_head", "用干燥背景布隔开头肩与带电货架", ["用布隔开头和电", "绝缘后用头顶", "头肩顶住带电架"], c => c.state.sceneId === "darkroom_alone" && c.state.player.pose !== "stand" && isNear(c, "frame", 120), { type: "scene_interaction", target: "frame", flags: { embodied_chain: true, body_supporting_frame: true, head_insulated: true }, progress: ["alone", 2], prophecy: ["P_HEAD_ABOVE_FRAME", "head_brace"], message: "干燥背景布夹在头肩与金属架之间。它会吸水，绝缘并非永久状态。" }),
  action("cinema_yao_align_shadow", "让苏遥用放映窗逐段核对影子与本人", ["核对影子和本人", "让苏遥看我的影子", "逐段确认轮廓"], c => c.state.sceneId === "cinema_yao" && c.nearNpcIds.includes("yao"), { type: "scene_interaction", target: "projector", flags: { yao_witness_chain: true, shadow_identity_checked: true, player_in_beam: true }, progress: ["yao", 2], trust: ["yao", 1], prophecy: ["P_SHADOW", "body"], message: "苏遥先看本人，再看影子；两个时刻之间的走位被明确保留下来。" }),
  action("cinema_luhui_echo_alibi", "与陆洄用回声和视线互换不在场证明", ["用回声互相证明", "陆洄听我我看他", "交换声音和视线"], c => c.state.sceneId === "cinema_luhui" && c.nearNpcIds.includes("luhui"), { type: "scene_interaction", target: "projector", flags: { luhui_mutual_alibi: true, echo_alibi: true }, progress: ["luhui", 2], trust: ["luhui", 1], message: "苏岚看见陆洄，陆洄只听见苏岚。互证成立，但两人证明的并不是同一种在场。" }),
  action("cinema_alone_register_frames", "按固定胶片速度记录连续遮挡", ["记录连续遮挡", "按胶片帧数证明", "连续曝光我的动作"], c => c.state.sceneId === "cinema_alone" && isNear(c, "projector", 140), { type: "scene_interaction", target: "projector", trace: "film_sequence", flags: { embodied_chain: true, double_exposure_ready: true, frame_rate_fixed: true }, progress: ["alone", 2], prophecy: ["P_TWO_RECORDS", "double_exposure"], message: "固定帧速下，每次遮挡都占用不同胶片位置。记录不命名苏岚，只保留同一行动链。" }),
