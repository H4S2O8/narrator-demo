import { ACTIONS } from "./data/actions.js?v=20260813f";
import { ACT1, ACT2, ACT3, OBJECTS, PAST, PROPHECIES, SCENE_VARIANTS, TUTORIAL, createWorldlines } from "./data/content.js?v=20260813f";
...
q('#past-table').innerHTML=Object.values(PAST).map(p=>`<tr><td>${p.id}</td><td>${p.text}</td><td>${p.axis} ∈ {${p.keep.join(' / ')}}</td><td>${p.tags.join('、')}</td><td>${p.backlash}</td></tr>`).join('');
