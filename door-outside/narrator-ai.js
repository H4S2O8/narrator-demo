(()=>{
  ...
  function setStatus(text,kind='idle'){
    ...
    if(statusLine) statusLine.textContent=kind==='ready'?'模型仅在本机浏览器运行':kind==='error'?text:'首次下载后会缓存在浏览器中';
  }
  ...
  async function installModel(){
    ...
    loading=(async()=>{
      if(!navigator.gpu)throw new Error('此浏览器暂不支持 WebGPU');
      ...
      engine=await webllm.CreateMLCEngine(MODEL_ID,{
        initProgressCallback:(report)=>{
          ...
          setStatus(`下载本地模型 ${pct}%`);
        }
      });
      setStatus('本地旁白模型已就绪','ready');
      return engine;
    })().catch(error=>{
  ...
  async function browserModel(p){
    ...
    try{
      const response=await engine.chat.completions.create({
        messages:[{role:'system',content:'你是严格的游戏动作解析器，只输出单个 JSON 对象。'},{role:'user',content:prompt}],
        temperature:0,
        max_tokens:96,
        response_format:{type:'json_object'}
      });
      const text=String(response.choices?.[0]?.message?.content||'').replace(/^```json|```$/g,'').trim();
  ...
  function mountInstaller(){
    ...
    panel.innerHTML='<button type="button">下载本地旁白模型（可选）</button><small>首次下载后会缓存在浏览器中</small>';
